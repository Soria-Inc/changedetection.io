...
def _request_with_redirects(method, url, *, headers, proxies, timeout, stream=False):
...
def fingerprint_file(url, previous, *, headers, proxies, timeout, now):
    previous = previous or {}
    head_session = head = None
    try:
        head_session, head, final_url = _request_with_redirects(
            'HEAD', url, headers=headers, proxies=proxies, timeout=timeout
        )
        ...
        get_session, response, final_url = _request_with_redirects(
            'GET', url, headers=headers, proxies=proxies, timeout=timeout, stream=True
        )
        try:
...
def fingerprint_files(urls, previous_state, *, headers, proxies, timeout):
    ...
    now = time.time()
    worker_limit = max(1, int(os.getenv('LINKED_FILE_HEAD_WORKERS', '8')))
